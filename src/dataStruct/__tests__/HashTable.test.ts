...
describe('HashTable', () => {
  ...
  it('should set, read and delete data with collisions', () => {
    ...
    const stringify = (value: { key: string; value: string | number }) =>
      `${value.key}:${value.value}`;

    expect(hashTable.buckets[0].toString(stringify)).toBe('c:earth');
    expect(hashTable.buckets[1].toString(stringify)).toBe('a:sky,d:ocean');
    expect(hashTable.buckets[2].toString(stringify)).toBe('b:sea');
